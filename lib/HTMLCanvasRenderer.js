...
HTMLCanvasRenderer.prototype.draw = function (items, scale) {
  ...
  var margin = 3;
  var numCellsPerRow = 4; //for image lists, grid layout
  var cellSize = Math.floor((w - 2*margin) / numCellsPerRow);
  ...
  for (var i = 0; i < items.length; i++) {
    ...
    if (e.type == 'texturelist') eh = 18 + 3 + Math.ceil(e.items.length/numCellsPerRow) * cellSize;
    ...
    if (e.type == 'slider') {
      ...
      ctx.fillStyle = 'rgba(255, 255, 255, 1)';
      ctx.fillText(e.title, dx + 4, dy + 13);
      var numRows = Math.ceil(e.items.length / numCellsPerRow);
      for (var j = 0; j < e.items.length; j++) {
        var col = j % numCellsPerRow;
        var row = Math.floor(j / numCellsPerRow);
