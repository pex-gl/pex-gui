...
GUI.prototype.drawTextures = function () {
  for (var i = 0; i < this.items.length; i++) {
    ...
    if (item.type == 'texture2D') {
      var bounds = new Rect(
        item.activeArea.x * this.scale * this.highdpi,
        item.activeArea.y * this.scale * this.highdpi,
        item.activeArea.width * this.scale * this.highdpi,
        item.activeArea.height * this.scale * this.highdpi
      );
